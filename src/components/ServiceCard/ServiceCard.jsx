...
export default function ServiceCard({service, index}) {
  ...
  return (
    ...
    //  mouse er moving er sathe sathe border dekhabe
         ...
         h-[300px] absolute z-0 transition-opacity duration-500 mix-blend-lighten ${visible ? 'opacity-70' : 'opacity-0'}`} style={{top: position.y -150, left: position.x -150}}>
            ...
            <div className='flex items-center gap-10 p-8 hover:p-7.5 hover:m-0.5 transition-all rounded-[10px] bg-white dark:bg-gray-900 z-10 relative'>
                <div className='bg-gray-100 rounded-full'>
                    <img src={service.icons} className='max-w-24 bg-white dark:bg-gray-900 rounded-full m-2' alt="" />
                </div>
                <div className='flex-1'>
                    <h3 className='font-bold text-start'>{ service.title}</h3>
                    <p className='text-sm mt-2 text-start'>{service.description}</p>
                </div>
            </div>
    </div>
  )
